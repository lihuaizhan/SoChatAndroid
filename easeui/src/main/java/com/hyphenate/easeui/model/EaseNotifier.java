...
public class EaseNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            String fname = "";
            if (message.ext().get("truename") != null) {
                fname = message.ext().get("truename") + ",";
            }
            String summaryBody = fname + msgs[6].replaceFirst("%1",
                    Integer.toString(fromUsersNum)).replaceFirst("%2", Integer.toString(notificationNum));
            if (notificationInfoProvider != null) {
                // lastest text
            ...
            BadgeUtil.setBadgeCount(appContext, notificationNum);
