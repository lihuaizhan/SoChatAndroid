...
public class VisitorListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<Visitor.DataBean.VuiListBean> list;
    ...
    //构造参数
    public VisitorListAdapter(Context context, List<Visitor.DataBean.VuiListBean> list, boolean isChange) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        Visitor.DataBean.VuiListBean vuiListBean = list.get(position);
        with = Glide.with(context);
        if (vuiListBean.getPicture().equals("baidu.com")) {
            ...
            with.load(vuiListBean.getPicture()).transform(new GlideCircleTransform(context)).into(holder1.visitorTou);
        }
        if(!isChange)
        {
            ...
            holder1.visitorTime.setText(TimeConvertUtil.getTimeInterval(vuiListBean.getVisitTime()));
        }
    ...
    //设置list返回长度
    public int setListSize(boolean isChange) {
        size = list.size();
        if (isChange) {
